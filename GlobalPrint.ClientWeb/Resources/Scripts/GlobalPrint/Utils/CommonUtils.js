...
(function (CommonUtils) {
			...
			answers: [
				{
					...
					handler: function () {
						var $image = $(".img-container > img");
						var data = $image.cropper("getData"); // getDataURL
						if (callback) {
							callback(data);
						}
	...
	CommonUtils.showModalQuestion = function (confuguration) {
		/// <summary>
		/// Ask user a question. 
		/// </summary>
		/// <param name="confuguration" type="object">{ question:text, answers:[{ text: text, handler: function() {} }, ...] } </param>

		var modalWindowId = "modal_" + Math.floor(Math.random() * 100000000000);
		var modalHtml =
            '<div class="modal fade lookup-wrapper" id="' + modalWindowId + '" tabindex="-1" role="dialog" aria-hidden="true">                             ' +
            '    <div class="modal-dialog">                                                                                                                                 ' +
            '        <div class="modal-content">                                                                                                                            ' +
            '            <div class="modal-body">                                                                                                                           ' +
